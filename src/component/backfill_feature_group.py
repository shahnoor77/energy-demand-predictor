...
from src.component.feature_group_config import FEATURE_GROUP_METADATA
...
def run():
    ...
    electricity_demand_data["date"] = pd.to_datetime(
        electricity_demand_data["date"], utc=True
    )

    # ✅ 2. Optional - only for inspection (do NOT overwrite `date`)
    electricity_demand_data["date_str"] = electricity_demand_data["date"].dt.strftime(
        "%Y-%m-%d %H:%M:%S"
    )

    # ✅ 3. Optional - for model features or logging
    electricity_demand_data["seconds"] = (
        electricity_demand_data["date"].astype("int64") // 10**6
    )
