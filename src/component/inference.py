...
def load_batch_of_features_from_store(current_date: pd.Timestamp) -> pd.DataFrame:
    ...
    print(
        f"Filtered out sub-regions that do not meet the required {config.N_FEATURES} records."
    )
...
def load_predictions_from_store(from_date: datetime, to_date: datetime) -> pd.DataFrame:
    ...
    predictions = predictions_fv.get_batch_data(
        start_time=from_date - timedelta(days=1), end_time=to_date + timedelta(days=1)
    )
